...
use halo2_base::{
    gates::{RangeChip, GateInstructions},
...
fn sm_test(
    ...
    let s = ctx.load_constant(scalar);
    let P = ecc_chip.assign_point(ctx, base);

    println!("ctx.before_num_to_bits.advice.len() = {}", ctx.advice.len());
    let s_le_bits = range.gate.num_to_bits(ctx, s, 254).into_iter().take(128).collect_vec();
    println!("ctx.after_num_to_bits.advice.len() = {}", ctx.advice.len());

    let sm = ecc_chip.scalar_mult::<G1Affine>(
        ctx,
        P,
        s_le_bits, //.limbs().to_vec(),
        1, // fq_chip.limb_bits,
        window_bits,
    );
    println!("ctx.after_sm.advice.len() = {}", ctx.advice.len());
...
fn run_test(base: G1Affine, scalar: Fq) {
...
fn test_grumpkin_sm_random() {
    ...
    run_test(G1Affine::random(&mut rng), Fq::random(&mut rng));
}
